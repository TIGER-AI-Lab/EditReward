...
def get_model_embeddings_parameters(model):
    """
    尝试返回模型的 embedding parameters 迭代器（或 None）。
    支持多种常见 API：
      - model.embed_tokens
      - model.get_input_embeddings()
      - model.word_embeddings
      - model.model.embed_tokens (nested)
    返回: iterable(parameters) 或 None
    """
    # 1) 优先使用 get_input_embeddings()（HF 标准接口）
    try:
    ...
    # 3) 有时嵌套在 model.<something>
    if hasattr(model, "__dict__"):
        for k, v in vars(model).items():
            # 如果发现是 Module 且看起来像 embedding
            try:
                from torch import nn
                if isinstance(v, nn.Module):
                    name = k.lower()
                    if "embed" in name or "word" in name or "token" in name:
...
def safe_set_embedding_requires_grad(model, requires_grad=False):
    """
    安全设置模型embedding层的 requires_grad。如果找不到 embedding，
    打印 DEBUG 信息但不抛异常。
    """
...
def create_model_and_processor(
    ...
    # print(f"model_config.pooling_strategy: {model_config.pooling_strategy}")
...
def train(config, local_rank=0, debug=False):
    ...
    if peft_lora_config.lora_enable:
        ...
        # NOTE: 20250910 Added: 使用安全函数尝试冻结 embedding
        if not safe_set_embedding_requires_grad(model_to_configure.model, False):
            # 作为兜底：尝试 model_to_configure.model.model 里再找（某些 wrapper 结构）
    ...
    if model_to_configure.rm_head_type == "ranknet_multi_head" or model_to_configure.rm_head_type == "ranknet_multi_head_regression":
        # 遍历 rm_heads 的所有子模块
    ...
    ##### NOTE: plan to use GenAI-Arena Bench for test
    test_set_dict = {}
