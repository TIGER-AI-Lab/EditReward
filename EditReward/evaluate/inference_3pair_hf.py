...
def run_pairwise_comparison_embedded(model, source_image, instruct_prompt, image_a, image_b, prompt_template, comparison_type):
    ...
    prompt = prompt.replace("<source_prompt>", "")  # Not available in 3pair data
    prompt = prompt.replace("<target_prompt>", "")   # Not available in 3pair data
...
def parse_pairwise_response(response, comparison_type):
    ...
    patterns = [
        ...
        r"(?:Final verdict|verdict|answer):\s*([ABCabc][><][ABCabc])(?:\s*$|\s*\.?\s*$)",
        r"\b([ABCabc]\s*[><]\s*[ABCabc])\b(?=\s*(?:\.|$|,|\s))",
        r"(?:Model\s+)?([ABCabc])\s+is\s+better",
    ]
    ...
    # Look for "A is better", "B is better", "C is better" patterns
    better_match = re.search(r"\b([ABCabc])\s+is\s+better", response, re.IGNORECASE)
    if better_match:
...
def evaluate_3pair_hf_example(model, example, prompt_template, example_idx):
    """Evaluate a single 3-pair example using Hugging Face dataset"""
    ...
    human_vote = example["ranking"]         # "A>B", "B>A", "A>C", "C>A", "B>C", "C>B"
    comparison_type = example["comparison_type"]  # "AvsB", "AvsC", "BvsC"
...
def process_single_hf_example(args):
    ...
    try:
        result = evaluate_3pair_hf_example(model, example, prompt_template, example_idx)
        return result
...
def main(
    ...
    """
    Parallel evaluation of model on 3-pair data from Hugging Face
    ...
    Note:
        ...
        Filters data to only include 3pair samples
        Group-level evaluation: all 3 comparisons in a group must be correct
    """
    
    print("="*60)
    print("EditReward-Bench 3-Pair Evaluation (Hugging Face)")
    print("="*60)
    ...
    try:
        ...
        # Filter to only 3pair samples
        data = [sample for sample in all_data if sample['dataset'] == '3pair']
        print(f"✅ Loaded {len(all_data)} total samples, filtered to {len(data)} 3pair samples")
    ...
    # Verify data
    if not data:
        print("❌ No 3pair samples found in dataset")
        return
    
    # Group samples by original sample (each group has 3 comparisons)
    print(f"\n🔍 Grouping samples...")
    ...
    # Verify each group has 3 comparisons
    ...
    if results_dir is None:
        results_dir = Path(__file__).parent / "results_3pair_hf_qwen2_5_7b"
    else:
        results_dir = Path(results_dir)
    
    results_file = results_dir / f"{model_name}_{template}_3pair_hf.json"
    ...
    for group_id, group_samples in group_results.items():
        # Check if all 3 comparisons in this group are correct
        if len(group_samples) == 3 and all(sample['is_correct'] for sample in group_samples):
            correct_groups += 1
