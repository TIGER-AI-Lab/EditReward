...
from EditReward.utils.training_utils import get_peft_state_non_lora_maybe_zero_3
...
class Qwen2_5_VLRewardModelBT_MultiHead(Qwen2_5_VLForConditionalGeneration):
    ...
    def _pool_logits(self, logits, input_ids, batch_size):
        """
        logits: tensor [B, L, N]
        returns pooled tensor [B, N] depending on self.reward_token
        """
        if self.reward_token == "last":
            # compute sequence_lengths like original code
            ...
            if self.config.pad_token_id is None:
                ...
                pooled = torch.stack([logits[i, : valid_lengths[i]].mean(dim=0) for i in range(batch_size)])
                return pooled
    ...
    def _run_single_batch_through_model_and_head(self, batch_dict, head_module):
        """
        batch_dict: a dict containing keys like input_ids, attention_mask, pixel_values, image_grid_thw ...
        head_module: nn.Module (one rm head)
        returns: pooled logits [B, out_dim]
        """
        ...
        # build inputs_embeds (same logic as before)
        ...
        with torch.autocast(device_type="cuda", dtype=torch.float32):
            logits = head_module(hidden_states)  # [B, L, N]

        # pooling
        pooled = self._pool_logits(logits, input_ids, batch_size)  # [B, N]
        return pooled
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        - 单 head 时：保持原有逻辑。
        - 多 head 时：支持传入 batch_dim1, batch_dim2, ...，每个 head 分别跑一遍。
        """
        ...
        # 多 head逻辑
        # -------------------------
        elif self.rm_head_type == "ranknet_multi_head" or self.rm_head_type == "ranknet_multi_head_regression":
            # print(f"self.pooling_strategy: {self.pooling_strategy}")
...
class VLMRewardTrainer(RewardTrainer):
    def __init__(self, loss_type="regular", loss_hyperparameters={}, tied_threshold=None, 
                 visualization_steps=500, max_viz_samples=4, rm_head_type="single_head", *args, **kwargs):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        if self.rm_head_type == "ranknet":
            rewards_A = model(return_dict=True, **inputs["batch_1"])["logits"]  ### [batch, 2]
            rewards_B = model(return_dict=True, **inputs["batch_2"])["logits"]  ### [batch, 2]
        elif self.rm_head_type == "ranknet_multi_head" or self.rm_head_type == "ranknet_share_head" or self.rm_head_type == "ranknet_multi_head_regression":
            # A-side (chosen) 
            # if self.rm_head_type == "ranknet_multi_head_regression": output_type: List
            rewards_A = model(return_dict=True,
                            batch_dim1=inputs["batch_1_dim1"],
                            batch_dim2=inputs["batch_1_dim2"])["logits"]  # fused [B, out_dim]

            # B-side (rejected)
            rewards_B = model(return_dict=True,
                            batch_dim1=inputs["batch_2_dim1"],
                            batch_dim2=inputs["batch_2_dim2"])["logits"]
        ...
        if self.loss_type == "bt":
            ...
            rewards = torch.stack([mean_chosen, mean_rejected], dim=1)  # [B, 2]

            # rewards = torch.stack([rewards_A.squeeze(-1),rewards_B.squeeze(-1)], dim=1)   # [batch, 2]
            # print(f"inputs[A_scores]: {inputs['A_scores']}.\n")
            # print(f"inputs[B_scores]: {inputs['B_scores']}.\n")
            scores = torch.stack([inputs["A_scores_overall"], inputs["B_scores_overall"]], dim=1)

            # print(f"rewards:{rewards}.\n")
            # print(f"scores:{scores}.\n")
            scores = (scores - 4.5)     # rescale
            # pdb.set_trace() 
        ...
        elif self.loss_type == "uncertainty_with_margin":
            # Add a margin hyperparameter, e.g., self.margin = 0.5
            margin = 0.57 # Or get it from self.margin
            batch_size = rewards_A.shape[0]
            mean_chosen = rewards_A[:, 0]
            mean_rejected = rewards_B[:, 0]
            sigma_chosen = torch.exp(rewards_A[:, 1])
            ...
            # The only change is here: subtract the margin from the samples
            loss = -torch.nn.functional.logsigmoid(z_samples - margin).mean()
        else:
        ...
        try:
            ...
            for i in range(batch_size):
                ...
                try:
                    ...
                    image_src = inputs['image_1'][i][0] if 'image_1' in inputs else None
                    image_A = inputs['image_1'][i][1] if 'image_1' in inputs else None
                    image_B = inputs['image_2'][i][1] if 'image_2' in inputs else None
                    ...
                    fig, axes = plt.subplots(nrows=1, ncols=3, figsize=(18, 8))  # 改成3列

                    fig.text(0.05, 0.05, f'Prompt:\n{prompt_A[:200]}{"..." if len(prompt_A) > 200 else ""}', 
                                 ha='left', va='bottom', fontsize=8, wrap=True,
                                 bbox=dict(boxstyle="round,pad=0.3", facecolor="lightblue", alpha=0.7))
                    
                    img_src_np = np.array(image_src)
                    if img_src_np.ndim == 3 and img_src_np.shape[0] == 3:  # CHW format
                        img_src_np = np.transpose(img_src_np, (1, 2, 0))
                    img_src_np = np.clip(img_src_np, 0, 1)  # Ensure values are in [0,1]
                    ...
                    axes[1].imshow(img_A_np)
                    axes[1].set_title(f'Image A - Score: {score_A_val:.3f}')
                    axes[1].axis('off')
                    ...
                    axes[2].imshow(img_B_np)
                
                    axes[2].set_title(f'Image B - Score: {score_B_val:.3f}')
                    axes[2].axis('off')
